...
1. [https://dashboard.stripe.com/test/products](https://dashboard.stripe.com/test/products) で Product を 2 件作成:
   - `GenbaHub Standard` — Recurring, ¥20,000 / month
   - `GenbaHub Pro`      — Recurring, ¥30,000 / month
   - 価格はオーナー最終確定前のたたき台（2026-07-05時点）。確定後は Stripe 側の Price を作り直し、
     `VITE_STRIPE_PRICE_STANDARD` / `VITE_STRIPE_PRICE_PRO` を新しい Price ID に差し替える。
