...
export type ActualCostsInput =
  ...
  | Array<{ taskId: string; cost: number }>;
...
function resolveActualCost(actualCosts: ActualCostsInput | undefined, tasks: ProgressTask[], fallbackAc: number): number {
  if (typeof actualCosts === "number") return actualCosts;

  if (Array.isArray(actualCosts)) {
    return actualCosts.reduce((sum, item) => sum + Math.max(0, item.cost), 0);
  }
...
export function calculateEarnedValue(tasks: ProgressTask[], budget: number, asOfDate = new Date().toISOString().slice(0, 10)): EarnedValueMetrics {
...
export function costPerformanceIndex(
  ...
  const metrics = calculateEarnedValue(tasks, budget, asOfDate);
  const actualCost = resolveActualCost(actualCosts, tasks, metrics.ac);
...
export function generateProgressReport(project: Project, tasks: ProgressTask[], asOfDate = new Date().toISOString().slice(0, 10)): string {
