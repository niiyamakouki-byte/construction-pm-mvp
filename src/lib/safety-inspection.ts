...
export function generateInspectionReport(checklist: InspectionChecklist): string {
  ...
  if (evaluation.criticalFailures.length > 0) {
    const critItems = evaluation.criticalFailures
      ...
      .join("");
    criticalSection = `<div style="background:#fef2f2;border:1px solid #ef4444;border-radius:4px;padding:12px;margin:16px 0;"><h3 style="color:#ef4444;margin:0 0 8px;">重大な不備</h3><ul>${critItems}</ul></div>`;
  }
...
<h1>安全点検レポート</h1>
<table>
<tr><td><strong>案件ID</strong></td><td>${escapeHtml(checklist.projectId)}</td></tr>
<tr><td><strong>点検者</strong></td><td>${escapeHtml(checklist.inspectedBy)}</td></tr>
<tr><td><strong>点検日</strong></td><td>${escapeHtml(checklist.date)}</td></tr>
<tr><td><strong>結果</strong></td><td style="color:${resultColor};font-weight:bold;">${overallResult}</td></tr>
<tr><td><strong>合格率</strong></td><td>${passPercent}%</td></tr>
</table>
${criticalSection}
<h2>チェックリスト詳細</h2>
<table>
