  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 rounded-xl bg-slate-100 p-1">
        {([
          { key: "pipeline" as Tab, label: "パイプライン", Icon: BarChart3 },
          { key: "customers" as Tab, label: "顧客一覧", Icon: Users },
          { key: "stats" as Tab, label: "サマリー", Icon: TrendingUp },
        ]).map(({ key, label, Icon }) => (
          <button
            key={key}
            type="button"
            onClick={() => setTab(key)}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-sm font-semibold transition-colors ${tab === key ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
          >
